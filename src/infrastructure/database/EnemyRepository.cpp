...
namespace constants {
    ...
    static constexpr const auto QUERY = "SELECT naam, omschrijving, minimumobjecten, maximumobjecten, levenspunten, aanvalskans, minimumschade, maximumschade FROM Vijanden WHERE naam = ?";
}
...
game_objects::EnemyObject* repository::EnemyRepository::get_enemy(const char* name) {
    ...
    if (sqlite3_prepare_v2(db_, constants::QUERY, -1, &stmt, nullptr) != SQLITE_OK) {
    ...
    while(sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        const auto enemy = new game_objects::EnemyObject(health, name, description);
        close();
        return enemy;
    }
